...
class RCSRepository(object):
    ...
    def write(self, fn, content):
        ...
        with directory_context(self.directory):
            with open(
                fn, "w", encoding=binary_encoding, errors="surrogateescape"
            ) as fp:
                fp.write(content)
    ...
    def stream(self, _module, _gitdir, outfile, more_opts=""):
        ...
        self.run_with_cleanup(
            'find -L {0} -name "*,v" | cvs-fast-export {1} {2} >{3}'.format(
                self.directory, vopt, more_opts, outfile
            )
        )
...
class CVSCheckout(object):
    ...
    def append(self, fn, content):
        ...
        with directory_context(self.directory):
            with open(
                fn, "a", encoding=binary_encoding, errors="surrogateescape"
            ) as fp:
                fp.write(content)
...
class ConvertComparison(object):
    ...
    def __init__(
        self, stem, repo=None, checkout=None, module=None, options="", showdiffs=False
    ):
        self.stem = stem
        self.repo = CVSRepository(repo if repo else stem + ".testrepo")
        self.checkout = self.repo.checkout(
            module, checkout if checkout else stem + ".checkout"
        )
    ...
    def compare_tree(self, legend, ref, success_expected=True):
        ...
        gitfiles = [
            fn[len(self.stem + ConvertComparison.SUFFIX) + 1 :] for fn in gitpaths
        ]
        cvsfiles.sort()
        gitfiles.sort()
        success = True
        if cvsfiles != gitfiles:
            ...
            success = False
        common = [
            (path, path.replace(".checkout/", ConvertComparison.SUFFIX + "/"))
            for path in cvspaths
            if path.replace(".checkout/", ConvertComparison.SUFFIX + "/") in gitpaths
        ]
        for (a, b) in common:
            if not filecmp.cmp(a, b, shallow=False):
                success = False
