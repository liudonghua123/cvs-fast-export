...
* Since .cvsignore files have a syntax (mostly) upward-compatible with that of
  ...
  Leading # characters on .cvsignore lines are escaped so git
  won't misunderstand them as comment leaders, and spaces in these
  lines are mapped to line feeds because CVS treats spaces as
  pattern separators.
...
These problems cannot be fixed in cvs-fast-export; they are inherent
to CVS.
